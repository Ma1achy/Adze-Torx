"""Small reproducible M3 training loop and masked structural metrics."""
...
def _tree_finite(tree) -> bool:
    return bool(
        jnp.all(
            jnp.asarray(
                [jnp.all(jnp.isfinite(cast(jax.Array, x))) for x in jax.tree_util.tree_leaves(tree)]
            )
        )
    )


def _boundary_metrics(predicted, target, observed):
    predicted = cast(jax.Array, predicted)
    target = cast(jax.Array, target)
    observed = cast(jax.Array, observed)
...
def _length_metrics(predicted, target, observed, config: StructureConfig):
    predicted = cast(jax.Array, predicted)
    target = cast(jax.Array, target)
    observed = cast(jax.Array, observed)
...
def evaluate(
    params, data: StructuredCarrier, keys: jax.Array, config: M3Config, train_config: TrainConfig
):
    ...
    if predictions.boundary_logits is not None:
        boundary_pred = jnp.argmax(predictions.boundary_logits, axis=-1)
        boundary_ce = -jnp.take_along_axis(
            jax.nn.log_softmax(predictions.boundary_logits), data.b[..., None], axis=-1
        )[..., 0]
        metrics.update({"boundary_cross_entropy": jnp.mean(boundary_ce)})
        metrics.update(
            {
                f"boundary_{k}": v
                for k, v in _boundary_metrics(boundary_pred, data.b, predictions.observed_b).items()
            }
        )
    if predictions.length_logits is not None:
        length_pred = jnp.argmax(predictions.length_logits, axis=-1)
        length_ce = -jnp.take_along_axis(
            jax.nn.log_softmax(predictions.length_logits), data.length[..., None], axis=-1
        )[..., 0]
        metrics.update({"length_cross_entropy": jnp.mean(length_ce)})
        metrics.update(
            {
                f"length_{k}": v
                for k, v in _length_metrics(
                    length_pred, data.length, predictions.observed_length, config.structure
                ).items()
            }
        )
    return metrics
...
def run(
    ...
    for step in range(train_config.steps):
        index_key = jax.random.key(train_config.seed + step)
        indices = jax.random.randint(
            index_key, (train_config.batch_size,), 0, train_data.h.shape[0]
        )
        batch = jax.tree_util.tree_map(lambda x, indices=indices: x[indices], train_data)
        value, grads = step_fn(
        ...
        if not bool(jnp.isfinite(value)) or not _tree_finite(grads):
            raise FloatingPointError("non-finite M3 loss or gradient")
    ...
    metrics["parameter_norm"] = jnp.sqrt(
        sum(jnp.sum(cast(jax.Array, x) ** 2) for x in jax.tree_util.tree_leaves(params))
    )
    return params, metrics


def class_baselines(data: StructuredCarrier) -> dict[str, float]:
    b_nonforced = data.b[:, 1:].reshape(-1)
    b_counts = jnp.bincount(b_nonforced, length=2)
    length_counts = jnp.bincount(data.length.reshape(-1), length=4)
    return {
        "boundary_majority": float(jnp.max(b_counts) / jnp.sum(b_counts)),
        "length_majority": float(jnp.max(length_counts) / jnp.sum(length_counts)),
        "boundary_prevalence": float(jnp.mean(b_nonforced)),
        "length_zero_fraction": float(jnp.mean(data.length == 0)),
    }
